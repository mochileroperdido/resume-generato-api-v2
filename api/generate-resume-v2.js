...
/**
 * Main Vercel serverless function handler (Version 2)
 */
  ...
  try {
    ...
    if (method === 'GET' && url === '/test-docx') {
      try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="test-resume-v2.docx"');
    ...
    if (method === 'POST') {
      console.log('Received POST request for resume generation (v2)');
      ...
      const filename = `resume-${safeName}-${timestamp}-v2.docx`;
    ...
    console.error(`Error in serverless function v2: ${error.message}`);
